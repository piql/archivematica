...
class TestHashsum(object):
    ...
    def test_compare_hashes_with_bad_files(self, mocker):
        ...
        objects_path = ""
        ret = hashsum.compare_hashes(objects_path)
        mock.assert_called_once_with("-c", "--strict", hash_file)
        assert (
    ...
    def test_write_premis_event_to_db(self):
        ...
        """
        # Values the job writes.
        group_type = "transfer_id"
        event_type = "fixity_check"
        event_outcome = "pass"
        # Values we will write.
        package_uuid = "e95ab50f-9c84-45d5-a3ca-1b0b3f58d9b6"
        event_detail = "suma de verificación validada: OK"
        write_premis_event_per_file(package_uuid, event_detail)
        kwargs = {"removedtime__isnull": True, group_type: package_uuid}
        ...
        for uuid_ in file_uuids:
            ...
            assert (
                event[0].event_detail == event_detail
